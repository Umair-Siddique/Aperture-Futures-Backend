...
# os.makedirs("processed_files_2001", exist_ok=True)
...
# def chunk_summary(summary_text, chunk_size=2500, chunk_overlap=250):
#     splitter = RecursiveCharacterTextSplitter(
#         chunk_size=chunk_size,
#         chunk_overlap=chunk_overlap,
#         separators=["\n\n", "\n", ".", "!"]
#     )
#     return splitter.split_text(summary_text)
...
#     try:
#         # Remove code block markers and inline comments
...
#     summary_chunks = chunk_summary(full_summary)

#     base_filename = os.path.basename(blob.name)

#     for idx, summary_chunk in enumerate(summary_chunks):
#         chunk_metadata = metadata.copy()
#         chunk_metadata["summary"] = summary_chunk
#         chunk_metadata["url"] = f"https://storage.googleapis.com/{bucket_name}/{blob.name}"

#         output_path = os.path.join("processed_files_2001", f"{base_filename}_chunk_{idx + 1}.json")

#         with open(output_path, "w", encoding="utf-8") as f:
#             json.dump(chunk_metadata, f, indent=2, ensure_ascii=False)

#     print(f"✅ Processed: {blob.name} into {len(summary_chunks)} chunks")
...
#     for blob in blobs:
#         try:
#             process_pdf_with_llm(blob, bucket_name)
#         except Exception as e:
#             print(f"❌ Failed processing {blob.name}: {e}")
...
# Ensure processed_files directory exists
os.makedirs("processed_files_2025", exist_ok=True)
...
def process_pdf_with_llm(blob, bucket_name):
    ...
    output_path = os.path.join("processed_files_2025", f"{os.path.basename(blob.name)}.json")
    with open(output_path, "w", encoding="utf-8") as f:
