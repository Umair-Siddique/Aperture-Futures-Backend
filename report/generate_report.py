...
def format_transcript_text(transcript: str) -> str:
    ...
    system_prompt = """You are a professional transcript formatter specializing in creating clean, well-structured meeting transcripts in markdown format.

Your task is to transform raw, error-prone transcripts into polished, readable markdown documents while preserving ALL original content.

CRITICAL RULES:
1. DO NOT summarize, paraphrase, or skip any content
2. Preserve ALL information from the original transcript
3. Output ONLY in English (translate any non-English text to English naturally)
4. Use proper markdown formatting with clear headings
5. Remove duplicate sentences/paragraphs (keep first occurrence)
6. Fix obvious typos, capitalization, and punctuation errors
7. Remove filler words (um, uh, like) and false starts only when they don't affect meaning
8. Clean up nonsensical sentences by correcting grammar while keeping the intended meaning
9. Identify and label speakers clearly when mentioned
10. Add section headings based on topic changes or speaker changes

FORMATTING REQUIREMENTS:
- Use markdown headings (##, ###) to structure the content
- Separate different speakers or topics with clear headings
- Use proper paragraphs with line breaks
- Use **bold** for emphasis where appropriate
- Use bullet points or numbered lists when listing items
- Format speaker names as: **[Speaker Name/Role]:**
...
- Topic/Agenda: [if mentioned]
- Participants: [if mentioned]

## [Topic/Section Name]

**[Speaker Name/Role]:**
[Clean, well-formatted content with proper paragraphs]
...
REQUIREMENTS:
- Output in English ONLY (translate any non-English text naturally)
- Use markdown format with clear headings (##, ###)
- Preserve ALL content - do not summarize or skip anything
- Clean up typos, grammar errors, and nonsensical sentences
- Remove duplicates and filler words (um, uh, like)
- Add section headings for different speakers or topics
- Format speaker names as **[Speaker Name]:**
...
{transcript}"""
            ...
            for i, chunk in enumerate(chunks):
                ...
                chunk_context = ""
                if i > 0:
                    chunk_context = "This is a continuation of a longer transcript. Maintain consistent formatting with previous sections. "
                if i < len(chunks) - 1:
